...
There are no stock photos in this build — product tiles and banners use
generated gradient/texture placeholders (see `--sport-tex`, `--street-tex`,
and `.tile.t1`–`.t6` in `style.css`) so the site works immediately with no
broken image links.
